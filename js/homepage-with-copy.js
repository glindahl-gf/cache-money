/* Extracted from homepage-with-copy.html */
...
/* =========================================================================
   1. HERO SUBJECT CYCLER — typewriter effect
   Types each word character-by-character, holds, deletes, advances. The
   blinking caret lives in CSS (`.subject-wrap::after`) so it keeps
   blinking even between phases without extra JS state.
   ========================================================================= */
const SUBJECTS = ["Player's", "Partner's", "Producer's", "Performer's", "Brand's"];

function startSubjectTyper(el, opts = {}) {
  if (!el) return;
  const TYPE_MS   = opts.typeMs   ?? 85;    // ms per char while typing
  const DELETE_MS = opts.deleteMs ?? 45;    // ms per char while deleting
  const HOLD_MS   = opts.holdMs   ?? 1400;  // dwell on fully-typed word
  const GAP_MS    = opts.gapMs    ?? 240;   // pause when empty before next word
  ...
  function step() {
    ...
    if (phase === 'typing') {
      ...
      wordIdx = (wordIdx + 1) % SUBJECTS.length;
      phase = 'typing';
      step();
    }
  }

  setTimeout(step, opts.startDelay ?? 0);
}

// Dark hero — starts immediately.
startSubjectTyper(qs('#subject'));
...
(() => {
  ...
  // Pool of source images — curated set under ./images/library/hero-section/.
  // Each visit shuffles independently, so the order looks fresh per load.
  // No null entries → no placeholder swatches. Filenames with spaces /
  // parens are URL-encoded so the CSS `url(...)` parser handles them.
  const POOL = [
    './images/library/hero-section/andrew-heald-n9RZ0dCunpY-unsplash.jpg',
    './images/library/hero-section/appic-Y__ZYcowSo0-unsplash.jpg',
    './images/library/hero-section/august-phlieger-CREqtqgBFcU-unsplash%20%281%29.jpg',
    './images/library/hero-section/jacek-dylag-fZglO1JkwoM-unsplash.jpg',
    './images/library/hero-section/james-lee-JB41zA4movk-unsplash.jpg',
    './images/library/hero-section/jeffrey-f-lin-6k0VD3xNw6U-unsplash.jpg',
    './images/library/hero-section/jonny-kennaugh-nPOtzvGLYW0-unsplash%20%281%29.jpg',
    './images/library/hero-section/keithjj-basketball-1558098_1920.jpg',
    './images/library/hero-section/keithjj-football-1464632_1920.jpg',
    './images/library/hero-section/logan-weaver-lgnwvr-31zFmHWBaDE-unsplash.jpg',
    './images/library/hero-section/mick-haupt-m0iXio5FF7M-unsplash.jpg',
    './images/library/hero-section/omar-ramadan-Z8hTy2q9dGY-unsplash.jpg',
    './images/library/hero-section/player-baseball-swing.jpg',
    './images/library/hero-section/sebastian-staines-mfzdRsWsiRA-unsplash.jpg',
  ];

  const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  const NUM_CARDS    = 20;       // dialed back from 30 — fewer compositor layers (less GPU pressure with 3–5 MB source JPGs) and wider angular spacing reduces visual overlap between neighbors
  const CYCLE_SEC    = 8.0;      // seconds — single shared cycle so every card moves at the same speed
  ...
  // Single portrait aspect — every card has identical dimensions
  const ASPECTS = [
    [3, 4],
  ];
  ...
  function buildCards() {
    ...
    for (let i = 0; i < NUM_CARDS; i++) {
      // Even angle distribution + tighter jitter — no clumping at one
      // direction, and adjacent cards stay close to their evenly spaced
      // angular slots so they don't drift on top of each other.
      const sector = (i / NUM_CARDS) * Math.PI * 2;
      const jitter = (Math.random() - 0.5) * (Math.PI * 2 / NUM_CARDS) * 0.5;
      const angle  = sector + jitter;

      // Shared cycle duration — every card travels at the same speed.
      // Phase offset uses a golden-ratio fraction (φ⁻¹ ≈ 0.6180) of the
      // cycle, so neighbouring angular slots land at phases ~0.6 cycles
      // apart and can't be at the same radius at the same time — kills
      // the chase-each-other overlap that random offsets produced.
      const cycleSec = CYCLE_SEC;
      const offsetSec = ((i * 0.6180339887) % 1) * cycleSec;
      const isFront = Math.random() < 0.5;

      // Uniform card dimensions — every card renders at the same size.
      // isFront is still used for z-ordering (above/below the headline).
      const ratio = ASPECTS[0];
      const baseSize = 240;
  ...
  function tick(now) {
    ...
    for (let i = 0; i < cards.length; i++) {
      ...
      // Tunnel-vision perspective: cards emerge from the dead-center
      // vanishing point at near-zero scale, accelerate outward along
      // their angle, and grow past 1.0 to feel like they rush past the
      // viewer. Power-curve easing on both distance and scale is what
      // gives the warp-speed sense of depth (linear was too flat — see
      // earlier safe-zone-perimeter implementation in git history).
      const tDist  = Math.pow(phase, 1.55);            // slow at start, fast at end
      const tSize  = Math.pow(phase, 1.85);
      const radius = maxRadius * 1.25 * tDist;          // overshoots viewport so cards exit fully
      const x = c.cosA * radius;
      const y = c.sinA * radius;
      const scale = 0.04 + tSize * 1.5;                 // 0.04 → 1.54
  ...
  if (reduceMotion) {
    // Static: place cards along their angular rays at varied phases so
    // the section reads as a depth-staggered swarm without motion.
    for (const c of cards) {
      const phase = (c.offsetSec % c.cycleSec) / c.cycleSec;
      const tDist = Math.pow(phase, 1.55);
      const tSize = Math.pow(phase, 1.85);
      const radius = maxRadius * 0.9 * tDist;
      const x = c.cosA * radius;
      const y = c.sinA * radius;
      const scale = 0.1 + tSize * 1.1;
      c.el.style.transform = `translate3d(${x.toFixed(1)}px, ${y.toFixed(1)}px, 0) scale(${scale.toFixed(2)})`;
  ...
  // Pause RAF unless the hero is meaningfully visible. Threshold 0.1
  // (was 0) so that the two heroes don't both animate at the seam where
  // a thin sliver of each is on-screen — that briefly doubled the work
  // and was visible as lag.
...
/* Section 6 (Editorial slideshow auto-rotator) was removed when the
   `proof / See-it-run` section was replaced by the case-studies bento. */
