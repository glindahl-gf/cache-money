...
/* --- block 2 of 2 --- */
/* Variant 2 — Editorial Cover hero rotator.
   Cycles Players / Leagues / Brands every 3.6s; cross-fades the background photo.
   Clicking an index item pauses the carousel for 8s and jumps to that subject. */
(() => {
  const rotator = document.getElementById('coverRotator');
  const index   = document.getElementById('coverIndex');
  if (!rotator || !index) return;
  const photos = Array.from(document.querySelectorAll('.cover-bg .cover-photo'));
  const words  = Array.from(rotator.querySelectorAll('.cover-word'));
  const items  = Array.from(index.querySelectorAll('li'));
  ...
  const apply = (nextIdx) => {
    ...
    cur = nextIdx;
    const key = KEYS[cur];
    photos.forEach(p => p.classList.toggle('is-current', p.dataset.key === key));
    words.forEach(w => {
    ...
    items.forEach(li => li.classList.toggle('is-current', li.dataset.key === key));
  };
  const tick      = () => apply((cur + 1) % KEYS.length);
  const startTimer = () => { stopTimer(); timer = setInterval(tick, 3600); };
  const stopTimer  = () => { if (timer) { clearInterval(timer); timer = null; } };

  items.forEach(li => {
    li.addEventListener('click', () => {
      const i = KEYS.indexOf(li.dataset.key);
      if (i === -1 || i === cur) return;
      apply(i);
      stopTimer();
